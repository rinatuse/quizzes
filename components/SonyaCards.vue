...
import { sonya } from '~/data/sonya.js'
...
const categories = computed(() => [...new Set(sonya.map(m => m.category))])

const filteredMeds = computed(() => {
  let result = selectedCategory.value ? sonya.filter(m => m.category === selectedCategory.value) : sonya
